...
class RGTAN(nn.Module):
    def __init__(self,
        ...
        # --- CHANGE START ---
        # Pass neigh_features=None to TransEmbedding to disable neighbor feature processing
        self.n2v_mlp = TransEmbedding(
            ref_df, device=device, in_feats_dim=in_feats, cat_features=cat_features, neigh_features=None)
        self.nei_feat_dim = 0
        # --- CHANGE END ---

        self.layers = nn.ModuleList()
        # --- CHANGE START ---
        # Adjust layer dimensions to not include `self.nei_feat_dim`
    ...
    def forward(self, blocks, features, labels, n2v_feat=None, neighstat_feat=None):
        cat_h, _ = self.n2v_mlp(n2v_feat, None)  # We pass None for neighstat_feat
        h = features + cat_h
